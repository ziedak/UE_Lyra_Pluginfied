...
void ULyraTabListWidgetBase::NativeOnInitialized() { Super::NativeOnInitialized(); }
...
void ULyraTabListWidgetBase::NativeDestruct()
{
	...
	{
		if (!TabInfo.CreatedTabContentWidget) { continue; }
...
{
	const FLyraTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate(
		[&](const FLyraTabDescriptor& TabInfo) -> bool{ return TabInfo.TabId == TabNameId; });
...
void ULyraTabListWidgetBase::SetTabHiddenState(const FName TabNameId, const bool bHidden)
{
	for (FLyraTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.TabId != TabNameId) { continue; }
...
bool ULyraTabListWidgetBase::RegisterDynamicTab(const FLyraTabDescriptor& TabDescriptor)
{
	...
	if (TabDescriptor.bHidden) { return true; }
...
void ULyraTabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FLyraTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		...
		if (TabInfo.CreatedTabContentWidget) { TabInfo.CreatedTabContentWidget->RemoveFromParent(); }
	}
...
void ULyraTabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
	...
	if (!IsDesignTime() && GetCachedWidget().IsValid()) { SetupTabs(); }

	Super::HandlePostLinkedSwitcherChanged();
}

void ULyraTabListWidgetBase::HandleTabCreation_Implementation(const FName TabId, UCommonButtonBase* TabButton)
{
	FLyraTabDescriptor* TabInfoPtr;

	FLyraTabDescriptor TabInfo;
	if (GetPreregisteredTabInfo(TabId, TabInfo)) { TabInfoPtr = &TabInfo; }
	else { TabInfoPtr = PendingTabLabelInfoMap.Find(TabId); }

	if (TabButton->GetClass()->ImplementsInterface(ULyraTabButtonInterface::StaticClass()))
	{
		if (ensureMsgf(TabInfoPtr,
		               TEXT(
...
{
	if (PreregisteredTabInfoArray.Num() > 0) { return GetActiveTab() == PreregisteredTabInfoArray[0].TabId; }
...
{
	if (PreregisteredTabInfoArray.Num() > 0) { return GetActiveTab() == PreregisteredTabInfoArray.Last().TabId; }

	return false;
}

bool ULyraTabListWidgetBase::IsTabVisible(const FName TabId) const
{
...
int32 ULyraTabListWidgetBase::GetVisibleTabCount() const
{
	int32 Result = 0;
	const int32 TabCount = GetTabCount();
	for (int32 Index = 0; Index < TabCount; Index++) { if (IsTabVisible(GetTabIdAtIndex(Index))) { Result++; } }
...
void ULyraTabListWidgetBase::SetupTabs()
{
	for (FLyraTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		...
		if (!GetTabButtonBaseByID(TabInfo.TabId))
		{
