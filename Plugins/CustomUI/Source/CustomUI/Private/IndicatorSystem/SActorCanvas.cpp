...
// Offsets for each direction that the arrow can point
...
public:
	...
	FORCEINLINE void SetRotation(const float InRotation) { Rotation = FMath::Fmod(InRotation, 360.0f); }

	FORCEINLINE float GetRotation() const { return Rotation; }

	virtual FVector2D ComputeDesiredSize(float) const override
	{
		if (Arrow) { return Arrow->ImageSize; }
...
EActiveTimerReturnType SActorCanvas::UpdateCanvas(double InCurrentTime, float InDeltaTime)
{
	...
	if (!OptionalPaintGeometry.IsSet()) { return EActiveTimerReturnType::Continue; }
	...
	if (IndicatorComponent == nullptr)
	{
		...
		if (IndicatorComponent)
		{
			...
			for (UIndicatorDescriptor* Indicator : IndicatorComponent->GetIndicators()) { OnIndicatorAdded(Indicator); }
		}
		else
		{
	...
	if (LocalPlayer)
	{
		...
		if (LocalPlayer->GetProjectionData(LocalPlayer->ViewportClient->Viewport, /*out*/ ProjectionData))
		{
			...
			{
				FSlot& CurChild = CanvasChildren[ChildIndex];
			...
			if (IndicatorsChanged) { Invalidate(EInvalidateWidget::Paint); }
		}
		else { SetShowAnyIndicators(false); }
	}
	else { SetShowAnyIndicators(false); }
...
void SActorCanvas::SetShowAnyIndicators(const bool bIndicators)
{
	if (bShowAnyIndicators == bIndicators) { return; }
	bShowAnyIndicators = bIndicators;

	if (bShowAnyIndicators) { return; }
			...
			if (bShouldClamp)
			{
				...
				if (!ClampRect.Contains(FIntPoint(ScreenPosition.X, ScreenPosition.Y)))
				{
					...
					for (int32 i = 0; i < EArrowDirection::MAX; ++i)
					{
						...
						{
							ClampDir = static_cast<EArrowDirection::Type>(i);
                            ...
                            bool bParentEnabled) const
{
	...
	for (const FArrangedWidget& CurWidget : ArrangedChildren.GetInternalArray())
	{
		...
		SWidget* MutableWidget = &CurWidget.Widget.Get();
...
FString SActorCanvas::GetReferencerName() const { return TEXT("SActorCanvas"); }
...
SActorCanvas::FScopedWidgetSlotArguments SActorCanvas::AddActorSlot(UIndicatorDescriptor* Indicator)
{
	...
	return FScopedWidgetSlotArguments{
		MakeUnique<FSlot>(Indicator), this->CanvasChildren, INDEX_NONE,
		[WeakCanvas](const FSlot*, int32){
			if (const TSharedPtr<SActorCanvas> Canvas = WeakCanvas.Pin()) { Canvas->UpdateActiveTimer(); }
		}
	};
}
...
int32 SActorCanvas::RemoveActorSlot(const TSharedRef<SWidget>& SlotWidget)
{
	...
	{
		if (!CanvasChildren.IsValidIndex(SlotIdx) || SlotWidget != CanvasChildren[SlotIdx].GetWidget()) { continue; }
                                    ...
                                    FVector2D& OutPaddingMax) const
{
	...
	const TSharedPtr<SWidget> CanvasHost = Indicator->CanvasHost.Pin();
	if (CanvasHost.IsValid()) { OutSize = CanvasHost->GetDesiredSize(); }
...
void SActorCanvas::UpdateActiveTimer()
{
	...
	{
		TickHandle = RegisterActiveTimer(0,
		                                 FWidgetActiveTimerDelegate::CreateSP(this, &SActorCanvas::UpdateCanvas));
	}
}
