...
#include "Foundation/LyraSimulatedInputWidget.h"
...
ULyraSimulatedInputWidget::ULyraSimulatedInputWidget(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer) { SetConsumePointerInput(true); }

#if WITH_EDITOR
const FText ULyraSimulatedInputWidget::GetPaletteCategory() { return LOCTEXT("PalleteCategory", "Input"); }
#endif // WITH_EDITOR
...
void ULyraSimulatedInputWidget::NativeConstruct()
{
	...
	if (UEnhancedInputLocalPlayerSubsystem* System = GetEnhancedInputSubsystem())
	{
		System->ControlMappingsRebuiltDelegate.AddUniqueDynamic(
			this, &ULyraSimulatedInputWidget::OnControlMappingsRebuilt);
	}
...
{
	if (UEnhancedInputLocalPlayerSubsystem* System = GetEnhancedInputSubsystem()) { return System->GetPlayerInput(); }
	return nullptr;
}
...
{
	if (UEnhancedPlayerInput* Input = GetPlayerInput()) { Input->FlushPressedKeys(); }
}
...
void ULyraSimulatedInputWidget::QueryKeyToSimulate()
{
	if (UEnhancedInputLocalPlayerSubsystem* System = GetEnhancedInputSubsystem())
	{
		...
		if (!Keys.IsEmpty() && Keys[0].IsValid()) { KeyToSimulate = Keys[0]; }
		else { KeyToSimulate = FallbackBindingKey; }
	}
}

void ULyraSimulatedInputWidget::OnControlMappingsRebuilt() { QueryKeyToSimulate(); }
