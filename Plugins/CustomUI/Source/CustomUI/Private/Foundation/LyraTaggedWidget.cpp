...
#include "Foundation/LyraTaggedWidget.h"
...
void ULyraTaggedWidget::SetVisibility(ESlateVisibility InVisibility)
{
	...
	if (bWantsToBeVisible) { ShownVisibility = InVisibility; }
	else { HiddenVisibility = InVisibility; }
	...
	const ESlateVisibility DesiredVisibility = (bWantsToBeVisible && !bHasHiddenTags)
		                                           ? ShownVisibility
		                                           : HiddenVisibility;
	if (GetVisibility() != DesiredVisibility) { Super::SetVisibility(DesiredVisibility); }
}
