...
{
	UpdateCrouchOffsetForTarget(DeltaTime);
	BlendCrouchOffset(DeltaTime);

	const FVector PivotLocation = GetPivotLocation() + CurrentCrouchOffset;
	FRotator PivotRotation = GetPivotRotation();

	UpdateViewTransform(PivotLocation, PivotRotation);
	// Adjust final desired camera location to prevent any penetration
	UpdatePreventPenetration(DeltaTime);
}


void UCameraMode_ThirdPerson::UpdateCrouchOffsetForTarget(float DeltaTime)
{
	...
	if (!TargetCharacterCDO)
	{
		UE_LOG(LogTemp, Warning, TEXT("TargetCharacterCDO is null. Using zero vector for crouch offset."));
		SetTargetCrouchOffset(FVector::ZeroVector);
		return;
	}
...
void UCameraMode_ThirdPerson::BlendCrouchOffset(const float DeltaTime)
{
...
void UCameraMode_ThirdPerson::UpdateViewTransform(const FVector& PivotLocation, FRotator& PivotRotation)
{
	...
	{
		// Get the target offset from the runtime float curves.
		// These curves are used to define the offset based on the pitch of the camera.
...
/*
 * ensures that the camera remains outside of objects by calculating a safe position based on the target actor
 * and its surroundings. It uses optional target actors, root components, and penetration avoidance feelers to
 * refine the camera's position dynamically.
 */
void UCameraMode_ThirdPerson::UpdatePreventPenetration(const float DeltaTime)
{
	// Skip penetration prevention if the feature is disabled, allowing the camera to move freely without collision checks.
	...
	const auto SafeLocation = CalculateSafeLocation(PPActor, PPActorRootComponent);
	...
	// Notify assist interfaces if the camera is penetrating
	const TArray<ICameraAssistInterface*> AssistArray = {TargetControllerAssist, TargetActorAssist, PPActorAssist};
	NotifyAssistInterfacesIfPenetrating(AssistArray);
}

FVector UCameraMode_ThirdPerson::CalculateSafeLocation(const AActor* PPActor,
...
void UCameraMode_ThirdPerson::PreventCameraPenetration(const AActor& ViewTarget,
                                                       ...
                                                       bool bUseSingleRayForPenetrationCheck)
{
	...
	auto DistBlockedPctThisFrame = 1.f;

	const int32 NumRaysToShoot = bUseSingleRayForPenetrationCheck
	...
	DistBlockedPct = UpdateDistBlockedPct(DistBlockedPct, DistBlockedPctThisFrame, HardBlockedPct, SoftBlockedPct,
	                                      DeltaTime);
...
FVector UCameraMode_ThirdPerson::CalculateRayTarget(const FVector& SafeLoc,
                                           ...
                                           float& DistBlockedPctThisFrame)
{
	...
	constexpr ECollisionChannel TraceChannel = ECC_Camera; //(Feeler.PawnWeight > 0.f) ? ECC_Pawn : ECC_Camera;
	...
	FHitResult Hit;
	const bool bHit = World->SweepSingleByChannel(Hit, SafeLoc, RayTarget, FQuat::Identity, TraceChannel, SphereShape,
	                                              SphereParams);
	...
	if (!bHit)
		return;
	if (IgnoreHit(Hit, SphereParams, ViewTarget))
		return;
	DistBlockedPctThisFrame = UpdateDistBlockedPctThisFrame(Hit, DistBlockedPctThisFrame, Feeler, RayTarget, SafeLoc);
}

bool UCameraMode_ThirdPerson::IgnoreHit(const FHitResult& Hit,
                                        FCollisionQueryParams& SphereParams,
                                        const AActor& ViewTarget) const
{
...
float UCameraMode_ThirdPerson::UpdateDistBlockedPct(const float DistBlockedPct,
                                                    const float DistBlockedPctThisFrame,
                                                    const float HardBlockedPct,
                                                    const float SoftBlockedPct,
                                                    const float DeltaTime) const
{
	auto BlockedPct = FMath::Clamp<float>(DistBlockedPctThisFrame, 0.f, 1.f);
	if (bResetInterpolation)
		return FMath::Clamp<float>(DistBlockedPctThisFrame, 0.f, 1.f);

	if (DistBlockedPct < DistBlockedPctThisFrame)
	{
		if (PenetrationBlendOutTime > DeltaTime)
			BlockedPct = DistBlockedPct + DeltaTime / PenetrationBlendOutTime * (DistBlockedPctThisFrame - DistBlockedPct);
		else
			BlockedPct = DistBlockedPctThisFrame;
	}
	else
	{
		if (DistBlockedPct > HardBlockedPct)
			BlockedPct = HardBlockedPct;
		else if (DistBlockedPct > SoftBlockedPct)
		{
			if (PenetrationBlendInTime > DeltaTime)
				BlockedPct = DistBlockedPct - DeltaTime / PenetrationBlendInTime * (DistBlockedPct - SoftBlockedPct);
			else
				BlockedPct = SoftBlockedPct;
		}
	}
...
void UCameraMode_ThirdPerson::NotifyAssistInterfacesIfPenetrating(TArray<ICameraAssistInterface*> AssistArray) const
{
...
void UCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
	...
	for (auto DebugActorsHi : DebugActorsHitDuringCameraPenetration)
	{
		if (!DebugActorsHi)
			continue;

		DisplayDebugManager.DrawString(
			FString::Printf(TEXT("HitActorDuringPenetration: %s"), *DebugActorsHi->GetName()));
	}
	// Draw the last debug time
