...
#include "Screens/LyraBrightnessEditor.h"
...
FReply ULyraBrightnessEditor::NativeOnAnalogValueChanged(const FGeometry& InGeometry,
                                                         const FAnalogInputEvent& InAnalogEvent)
{
	    ...
	    BrightnessEditor::JoystickDeadZone)
	{
		const float SafeZoneMultiplier = FMath::Clamp(
			...
			BrightnessEditor::SafeZoneChangeSpeed, 0.0f, 1.0f);
...
void ULyraBrightnessEditor::HandleInputModeChanged(const ECommonInputType InInputType) const
{
	...
	RichText_Default->SetText(FText::Format(LOCTEXT("BrightnessAdjustInstructions",
	                                                "Use <text color=\"FFFFFFFF\" fontface=\"black\">{0}</> to adjust the brightness"), KeyName));
}
