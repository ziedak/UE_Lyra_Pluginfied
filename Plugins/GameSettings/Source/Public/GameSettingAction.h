...
public:
	...
	DECLARE_EVENT_TwoParams(UGameSettingAction, FOnExecuteNamedAction, UGameSetting* /*Setting*/,
	...
	void SetNamedAction(const FGameplayTag Value) { NamedAction = Value; }

	bool HasCustomAction() const { return CustomAction.IsBound(); }
	void SetCustomAction(const UGameSettingCustomAction& InAction) { CustomAction = InAction; }
	 ...
	 */
	void SetDoesActionDirtySettings(const bool Value) { bDirtyAction = Value; }
