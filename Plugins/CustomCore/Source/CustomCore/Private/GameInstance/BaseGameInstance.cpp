...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BaseGameInstance)
...
void UBaseGameInstance::Init()
{
	...
	{
		ComponentManager->RegisterInitState(InitStateTags::SPAWNED, false, FGameplayTag());
		ComponentManager->RegisterInitState(InitStateTags::DATA_AVAILABLE, false, InitStateTags::SPAWNED);
		ComponentManager->RegisterInitState(InitStateTags::DATA_INITIALIZED, false, InitStateTags::DATA_AVAILABLE);
		ComponentManager->RegisterInitState(InitStateTags::GAMEPLAY_READY, false, InitStateTags::DATA_INITIALIZED);
	}
	...
	if (Lyra::bUseDTLSEncryption)
	{
		...
		if (PlayerController && PlayerController->PlayerState && PlayerController->PlayerState->GetUniqueId().IsValid())
		{
			...
			else
			{
				...
				if (Cert.IsValid())
				{
					const TArrayView<const uint8> Fingerprint = Cert->GetFingerprint();
